...
2. Launch N against a loopback fixture containing N+1:
   ...
   bun run updater:e2e:fixture -- run \
     --arch x64 \
     --current "/absolute/path/Hao Work-N-linux-x86_64.AppImage" \
     --next "/absolute/path/Hao Work-N+1-linux-x86_64.AppImage" \
     --version N+1 \
...
Normal packages omit the build-time marker and use the Hao Work production release provider.
