...
const PACKAGED_APP_USER_MODEL_ID = 'com.haowork.desktop';
const DEV_APP_USER_MODEL_ID = 'com.haowork.desktop.dev';
...
// that output never lands in ~/Library/Logs/Hao Work/main.log and we
...
const CHANGELOG_URL = 'https://raw.githubusercontent.com/skvdhshuk-blip/hao-work/main/CHANGELOG.md';
const GITHUB_BUG_REPORT_URL = 'https://github.com/skvdhshuk-blip/hao-work/issues/new?template=bug_report.yml';
const GITHUB_FEATURE_REQUEST_URL = 'https://github.com/skvdhshuk-blip/hao-work/issues/new?template=feature_request.yml';
...
const quitConfirmationMessage = () => {
  ...
  return `Hao Work detected ${reasons.join(', ')}. Quitting now will stop sidecar/background processes and may interrupt pending work.`;
};
...
const buildStartupSplashHtml = () => {
  ...
  <body>
    <div class="stack">
      <svg width="120" height="120" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Hao Work loading icon">
...
const parseConnectPairingDeepLinkPayload = (raw) => {
  ...
  try {
    ...
    return {
      pairingId,
      secret,
      label: typeof payload.label === 'string' && payload.label.trim() ? payload.label.trim() : 'Hao Work',
...
const confirmConnectDeepLink = async (payload) => {
  ...
  const options = {
    type: 'warning',
    title: 'Connect to Hao Work server?',
